...
int main() {
    ...
    cout << "\nStudent with Highest Attendance: Student " 
         << studentWithMax + 1 << " (" << maxAttendance << " Days)" << endl;

    int minAttendance = totalDay[0];
    int dayWithMin = 0;
    for (int j = 1; j < 7; j++) {
        if (totalDay[j] < minAttendance) {
            minAttendance = totalDay[j];
            dayWithMin = j;
        }
    }

    cout << "Day with Lowest Overall Attendance: Day " << dayWithMin + 1 << endl;
